        ...
        <form action="{{route('admin.employee-leave-allocation.update', $model->id)}}" method="post" id="content_form">
            ...
            <div class="row">
                ...
                @foreach ($model->allocation_details as $type)
                    <div class="col-md-6 form-group">
                        <label for="leave_type_{{$type->id}}">{{$type->leave_type->name}} <span class="text-danger">*</span></label>
                        <input type="text" name="leave_type[{{$type->id}}]" value="{{$type->allotted}}" id="leave_type_{{$type->leave_type->id}}" class="form-control" placeholder="Leave Allotted" required>
                    </div>
                @endforeach
                ...
                <div class="col-md-12 form-group">
                    <label for="name">{{_lang('Description')}} <span class="text-danger">*</span></label>
