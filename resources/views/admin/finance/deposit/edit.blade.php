<div class="card">
    <div class="card-header">
        <h6>{{_lang('Update Deposit Information')}}</h6>
    </div>
    <div class="card-body">
        <form action="{{route('admin.finance.deposit.update', $model->id)}}" method="post" id="content_form">
            @csrf
            @method('PATCH')
                <div class="row">
                    {{-- Account ID --}}
                    ...
                    <div class="col-md-12 form-group">
                        ...
                        <select name="category" id="category" class="form-control select" data-placeholder="Select One" required data-parsley-errors-container="category_error">
                            <option value="">{{ _lang('Select One') }}</option>
                            <option {{ $model->category == 'Envato' ? 'selected' : '' }} value="Envato">{{ _lang('Envato') }}</option>
                            <option {{ $model->category == 'Salary' ? 'selected' : '' }} value="Salary">{{ _lang('Salary') }}</option>
                            <option {{ $model->category == 'Other Income' ? 'selected' : '' }} value="Other Income">{{ _lang('Other Income') }}</option>
                            <option {{ $model->category == 'Interest Income' ? 'selected' : '' }} value="Interest Income">{{ _lang('Interest Income') }}</option>
                            <option {{ $model->category == 'Part Time Income' ? 'selected' : '' }} value="Part Time Income">{{ _lang('Part Time Income') }}</option>
                            <option {{ $model->category == 'Regular Income' ? 'selected' : '' }} value="Regular Income">{{ _lang('Regular Income') }}</option>
                            <option {{ $model->category == 'Other' ? 'selected' : '' }} value="Other">{{ _lang('Other') }}</option>
                        </select>
